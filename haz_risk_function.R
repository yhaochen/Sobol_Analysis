haz_risk_run<-function(i){
  ...
  mean_hazard <- mean(houses_table[,3], na.rm = T)
  ...
  damaged_houses <- damaged_houses[damaged_houses>1,]
  mean_damage_USD <- if(length(damaged_houses>0)) mean(damaged_houses) else 0
  ...
  outputs[1, 3] <- mat[i,"Z"]
  outputs[1, 4] <- mat[i,"W"]
  outputs[1, 5] <- mat[i,"n_ch"]
  outputs[1, 6] <- mat[i,"n_fp"]
  outputs[1, 7] <- mat[i,"DEM"]
  outputs[1, 8] <- mat[i,"V"]
  outputs[1, 9] <- mat[i,"X"]
  outputs[1, 10] <- mean_hazard
  outputs[1, 11] <- mean_damage_USD
  outputs[1,12] <- length(damaged_houses)
  ...
  colnames(outputs) <-
    c("run no.","Q","Z","W","n_ch","n_fp","DEM","V","X"
      ,"mean hazard (m)", "mean risk (USD)","n damaged houses")
