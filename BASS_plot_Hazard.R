...
for (seed_num in 1:3){
  for (size_num in 1:6){
    load(paste(folder,"/sensitivity_Hazard_",seed_num,"_",size_num,sep=""))
    
    # Each index is the mean of the last 100 saved MCMC iterations
    # Total-order indices:
    st <- colMeans(S_Hazard$T[c(400:499), ])
    
    # First-order indices:
    s1 <- colMeans(S_Hazard$S[c(400:499),c(1:n_params)])
    
    # second-order indices:
    s2 <- S_Hazard$S[c(400:499),c((n_params+1):(n_params+n_params*(n_params-1)/2))]
    s2 <- upper.diag(colMeans(s2))
    
    pdfname <- paste("radial_plot_hazard_",seed_num,"_",size_num,".pdf",sep="")
